...
export function BulkUploadModal({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/customers/bulk_upload/', {
        method: 'POST',
        body: formData,
      });
      ...
      >
        ...
        <div className="flex justify-between items-center p-8">
          <div>
            ...
            >
              Upload Excel file with customer names
            </p>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
            ...
            <div className="flex gap-2">
              ...
              <div className="text-sm">
                ...
                <ul className="list-disc list-inside space-y-1 text-xs opacity-90">
                  <li>File must have one column titled "LIST OF CUSTOMERS"</li>
                  <li>Each row should contain one customer name</li>
          ...
          {uploadResult && (
            <div className="space-y-4">
              ...
              {/* Imported Customers */}
              {uploadResult.imported_customers.length > 0 && (
                <div>
                  <h3 className="text-sm font-medium mb-2">Successfully Imported ({uploadResult.imported_customers.length})</h3>
                  <div className={`max-h-48 overflow-y-auto rounded-lg border ${
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {uploadResult.imported_customers.map((customer, idx) => (
                          <tr key={idx} className={`border-t ${
                            resolvedTheme === "dark"
                              ? "border-gray-700"
                              : "border-gray-200"
                          }`}>
                            <td className="px-3 py-2">{customer.row}</td>
