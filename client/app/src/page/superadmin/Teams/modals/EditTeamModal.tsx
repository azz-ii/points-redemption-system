...
export function EditTeamModal({
  ...
  const [pendingMarketingAdminId, setPendingMarketingAdminId] = useState<number | null>(
    null
  );
      ...
      >
        ...
        <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            ...
            <select
              value={editTeam.marketing_admin ?? ""}
              onChange={(e) => {
                const value = e.target.value ? Number(e.target.value) : null;
                console.log("DEBUG EditTeamModal: Marketing Admin changed", {
                  rawValue: e.target.value,
                  parsedValue: value,
                });

                // Check if this marketing admin is already assigned to other teams (excluding current team)
                if (value) {
                  const existingTeams = teams.filter(
                    (t) => t.marketing_admin === value && t.id !== team.id
                  );
                  if (existingTeams.length > 0) {
                    console.log(
                      "DEBUG EditTeamModal: Marketing Admin already assigned to",
                      existingTeams.length,
                      "other team(s)"
                    );
                ...
                setEditTeam({ ...editTeam, marketing_admin: value });
              }}
              className={`w-full px-3 py-2 rounded border ${
                resolvedTheme === "dark"
                  ? "bg-gray-800 border-gray-600 text-white"
        ...
        <div className="fixed inset-0 flex items-center justify-center z-60 p-4 bg-black/50 backdrop-blur-sm">
          ...
          >
            ...
            <div className="flex items-start gap-3 p-6 border-b border-gray-700">
              ...
              <div className="flex-1">
                <h3 className="text-lg font-semibold">
                  Marketing Admin Already Assigned
                </h3>
                <p className="text-sm text-gray-400 mt-1">
                  This marketing admin is already assigned to{" "}
                  {
                    teams.filter(
                      (t) =>
                        t.marketing_admin === pendingMarketingAdminId && t.id !== team.id
                    ).length
                  }{" "}
                  other team(s)
                </p>
            ...
            <div className="p-6">
              ...
              >
                {(() => {
                  const admin = marketingAdmins.find(
                    (a) => a.id === pendingMarketingAdminId
                  );
                  const assignedTeams = teams.filter(
                    (t) => t.marketing_admin === pendingMarketingAdminId && t.id !== team.id
                  );
                  return (
                    <>
                      <span className="font-semibold">
                      ...
                      is currently the marketing admin for:
                      <ul className="mt-2 ml-4 space-y-1">
                        {assignedTeams.map((t) => (
                          <li key={t.id} className="text-sm">
                            • {t.name}
                      ...
                      <p className="mt-3">
                        Do you still want to assign them as the marketing admin for{" "}
                        <span className="font-semibold">{team.name}</span>?
                      </p>
                    </>
                  );
            ...
            <div className="p-6 border-t border-gray-700 flex justify-end gap-3">
              <button
                onClick={() => {
                  console.log(
                    "DEBUG EditTeamModal: Marketing confirmation cancelled"
                  );
              ...
              <button
                onClick={() => {
                  console.log(
                    "DEBUG EditTeamModal: Marketing confirmation accepted",
                    pendingMarketingAdminId
                  );
                  setEditTeam({ ...editTeam, marketing_admin: pendingMarketingAdminId });
                  setShowMarketingConfirmation(false);
                  setPendingMarketingAdminId(null);
                }}
                className="px-6 py-3 rounded-lg font-semibold transition-colors bg-yellow-600 hover:bg-yellow-700 text-white"
