...
function Redemption() {
  ...
  const lastUpdatedLabel = useMemo(() => {
    if (!dataUpdatedAt) {
      return "Auto-refreshes every 30s";
    }
    return `Auto-refreshes every 30s • Updated ${new Date(dataUpdatedAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}`;
  }, [dataUpdatedAt]);
  ...
  const [selectedCancelRequest, setSelectedCancelRequest] =
    useState<RedemptionItem | null>(null);
  ...
  const pageSize = 7;
  const { filteredRequests, totalPages, safePage, paginatedRequests } =
    useMemo(() => {
  ...
  const handleCancelConfirm = async (reason: string, remarks: string) => {
    ...
    try {
      await handlerRequestsApi.cancelRequest(
