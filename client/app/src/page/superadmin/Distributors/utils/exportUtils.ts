...
export interface ExportColumn {
  key: keyof Distributor | "formatted_date";
  label: string;
  enabled: boolean;
}

export type SortField = keyof Distributor | "formatted_date";
 ...
 */
function getCellValue(distributor: Distributor, key: ExportColumn["key"]): string | number {
...
/**
 * Generate PDF export of distributors
 */
export function generatePDF(distributors: Distributor[], options: ExportOptions): void {
  const { columns, sortField, sortDirection, filename = "distributors_export" } = options;
  ...
  // Sort distributors
  const sortedDistributors = sortDistributors(distributors, sortField, sortDirection);

  // Prepare table data
  const headers = enabledColumns.map((col) => col.label);
  const rows = sortedDistributors.map((distributor) =>
    enabledColumns.map((col) => String(getCellValue(distributor, col.key)))
  );
  ...
  // Add title
  const title = "Distributors Export";
  ...
  // Add record count
  doc.text(`Total Records: ${sortedDistributors.length}`, pageWidth / 2, 34, {
    align: "center",
  });
...
/**
 * Generate Excel export of distributors
 */
export function generateExcel(distributors: Distributor[], options: ExportOptions): void {
  const { columns, sortField, sortDirection, filename = "distributors_export" } = options;
  const enabledColumns = columns.filter((col) => col.enabled);
  ...
  const headers = enabledColumns.map((col) => col.label);
  const rows = sortedDistributors.map((distributor) =>
    enabledColumns.map((col) => getCellValue(distributor, col.key))
  );
  ...
  // Add metadata sheet
  const metaData = [
    ["Distributors Export Report"],
    [],
    ["Generated On", new Date().toLocaleString()],
    ["Total Records", sortedDistributors.length],
...
/**
 * Export distributors using specified options
 */
export function exportDistributors(distributors: Distributor[], options: ExportOptions): void {
  if (options.format === "pdf") {
    generatePDF(distributors, options);
  } else {
    generateExcel(distributors, options);
  }
}
